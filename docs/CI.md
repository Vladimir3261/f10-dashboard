...
| Version bump guard (PRs only) | `git fetch origin $BASE && python3 tools/check_mapping_versions.py --against origin/$BASE` | A mapping (or `config/modes.yaml`) whose *content* changed relative to the PR base must have incremented its version; otherwise two different decodes would share one version number in the lake. Comment-only edits pass (content is compared as parsed). Needs `fetch-depth: 0`. Runs only on pull requests — on a push to `master` there is no base to compare against. |
| Research pipeline | `python3 -m research.build --evidence-only` | The importers, model validation, candidate gate and conflict detection run on the committed evidence files alone. Writes only the gitignored normalized output; does not rewrite the tracked reports (those cover the cached sources too; `--reports-only` refuses on an evidence-only set for the same reason, and `tests/research/test_tracked_reports.py` pins the tracked coverage report to the full-build view). Proves the pipeline is deterministic and self-contained without the licence-withheld source cache. |
