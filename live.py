...
#: the complete list of what /s/ may serve beyond SHARE_ALLOWED; index.html
#: references them by RELATIVE href, so the same page works under "/" and
#: under the share prefix without rewriting.
DASHBOARD_ASSETS = {
...
def read_dashboard_asset(name: str) -> bytes:
    """One file from dashboard/, by bare file name (no path components)."""
    if "/" in name or name.startswith("."):
        raise ValueError("asset names are bare file names: %r" % name)

    return (DASHBOARD_DIR / name).read_bytes()


PAGE = read_dashboard_asset("index.html").decode("utf-8")

ASSETS = {
    path: read_dashboard_asset(path.lstrip("/")) for path in DASHBOARD_ASSETS
}
...
def make_handler(
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def _headers(self, ctype: str, extra: Optional[Dict] = None):
            ...
            # no-store unless the caller says otherwise (the assets do)
        ...
        def _asset(self, path: str) -> None:
            """
            One file from dashboard/, under either prefix.

            `no-cache` (not `no-store`): the browser may keep a copy but
            must revalidate it on every load, so a `git pull` + restart
            on the Pi can never leave a stale app.js talking to a new
            API - the failure this header exists to prevent - while an
            unchanged file still answers 304 to a matching ETag.
            """
            payload = ASSETS[path]
            etag = '"%s"' % hashlib.sha1(payload).hexdigest()[:16]
            ...
            self._body(DASHBOARD_ASSETS[path], payload,
        ...
        def _serve_share(self, path: str, query: Dict) -> None:
            ...
            #
            # The page's own assets ride along with the page: the same
            # token gate, the same files as under "/", and nothing else -
            # DASHBOARD_ASSETS is the complete list, and it is enumerated
            # by a test against what index.html actually references.
            #
            if sub_path in DASHBOARD_ASSETS:
                self._asset(sub_path)
                return
